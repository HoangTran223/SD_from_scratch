...
def load_from_standard_weights(input_file: str, device: str) -> dict[str, dict[str, object]]:
    ...
    elif input_file.endswith(".ckpt"):
        original_model = torch.load(input_file, map_location=device, weights_only=False)["state_dict"]
    else:
        raise ValueError("Unsupported file format. Expect .safetensors or .ckpt")
    ...
    for k, v in original_model.items():
        if k.startswith("first_stage_model.encoder."):
            key = k.replace("first_stage_model.encoder.", "")
            converted["encoder"][key] = v
        elif k.startswith("first_stage_model.decoder."):
            key = k.replace("first_stage_model.decoder.", "")
            converted["decoder"][key] = v
        elif k.startswith("model.diffusion_model."):
            key = k.replace("model.diffusion_model.", "")
            converted["diffusion"][key] = v
        elif k.startswith("cond_stage_model."):
            key = k.replace("cond_stage_model.", "")
            converted["clip"][key] = v
        else:
            # Nếu muốn debug thêm:
            # print(f"[SKIP] {k}")
            continue
